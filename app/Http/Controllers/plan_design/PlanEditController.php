...
class PlanEditController extends Controller
{
    ...
    public function planeditDelete(Request $request)
    {
        if (isset($request->plan_id)) {
            $get_parent = DB::table('PLAN_BENEFIT_TABLE')->where('PLAN_ID', $request->plan_id)->first();
            $this->auditMethod('DE', json_encode($get_parent), 'PLAN_BENEFIT_TABLE');
            $get_child = DB::table('plan_table_extensions')->where('PLAN_ID', $request->plan_id)->first();
            $this->auditMethod('DE', json_encode($get_parent), 'plan_table_extensions');

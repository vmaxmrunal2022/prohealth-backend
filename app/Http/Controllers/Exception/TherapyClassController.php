...
use AuditTrait;
        ...
        if ($request->add_new == 1) {
            ...
            if ($validator->fails()) {
                ...
                if($add_names){
                    ...
                    $add_1 = DB::table('TC_EXCEPTION_LISTS')
                    ->insert([
            ...
            $save_audit = $this->auditMethod('IN', $record_snapshot, 'TC_EXCEPTION_LISTS');
            $add_parent = DB::table('TC_EXCEPTIONS')
            ->where('ther_class_exception_list',$request->ther_class_exception_list)
            ->first();

            $save_audit_parent = $this->auditMethod('IN', json_encode($add_parent), 'TC_EXCEPTIONS');
            return $this->respondWithToken($this->token(), 'Record  new Added Successfully', $add);

    
                // $add_my = DB::table('TC_EXCEPTION_LISTS')->where('ther_class_exception_list', 'like', '%' . $request->ther_class_exception_list . '%')->first();
                // return $this->respondWithToken($this->token(), 'Record Added Successfully', $add_my);
            ...
            if ($validator->fails()) {
                ...
                $save_audit = $this->auditMethod('UP', $record_snapshot, 'TC_EXCEPTION_LISTS');
                $get_names = DB::table('TC_EXCEPTIONS')
                    ->where(DB::raw('UPPER(ther_class_exception_list)'), strtoupper($request->ther_class_exception_list))
                    ->first();

                $save_audit_child = $this->auditMethod('UP', json_encode($get_names), 'TC_EXCEPTIONS');
                ...
                elseif($request->update_new == 1){
                    ...
                    if(count($checkGPI) >=1){
                        ...
                        $update_child = DB::table('TC_EXCEPTION_LISTS')
                        ->where(DB::raw('UPPER(ther_class_exception_list)'), strtoupper($request->ndc_exception_list))
                        ->where(DB::raw('UPPER(therapy_class)'), strtoupper($request->ndc))
                        ->first();
                    $record_snapshot = json_encode($update_child);
                    $save_audit = $this->auditMethod('IN', $record_snapshot, 'TC_EXCEPTION_LISTS');
                    $get_parent = DB::table('TC_EXCEPTIONS')
                        ->where(DB::raw('UPPER(ther_class_exception_list)'), strtoupper($request->ndc_exception_list))
                        ->first();
                    $save_audit_parent = $this->auditMethod('UP', json_encode($get_parent), 'TC_EXCEPTIONS');
    ...
    public function delete_therapy(Request $request)
    {
        if (isset($request->ther_class_exception_list) && isset($request->therapy_class)&& isset($request->effective_date)) {
            ...
            $save_audit_delete = $this->auditMethod('DE', json_encode($get_exceptions_lists), 'TC_EXCEPTION_LISTS');
            ...
            $get_exceptions_lists1=  DB::table('TC_EXCEPTION_LISTS')
            ->select('ther_class_exception_list')
           ->where('ther_class_exception_list', $request->ther_class_exception_list)
          
           ->get();

            $get_exceptions_lists2=  DB::table('TC_EXCEPTIONS')
           ->where('ther_class_exception_list', $request->ther_class_exception_list)
           ->first();
           ...
           foreach ($get_exceptions_lists1 as $excption) {
            ...
            $save_audit_delete1 = $this->auditMethod('DE', json_encode($excption), 'TC_EXCEPTION_LISTS');
  ...
  $save_audit_delete2 = $this->auditMethod('DE', json_encode($get_exceptions_lists2), 'TC_EXCEPTIONS');

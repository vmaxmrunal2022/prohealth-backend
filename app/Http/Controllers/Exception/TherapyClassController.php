    ...
    public function add(Request $request)
    {
        ...
        if ($request->new) {
            ...
            if ($validator->fails()) {
                ...
                if ($request->ther_class_exception_list && $request->ndc) {
                    $count = DB::table('TC_EXCEPTIONS')
                        ->where(DB::raw('UPPER(ther_class_exception_list)'), strtoupper($request->ther_class_exception_list))
                    ...
                    if ($count <= 0) {
                        ...
                        $updateProviderExceptionData = DB::table('TC_EXCEPTIONS')
                            ->where('ther_class_exception_list', $request->ther_class_exception_list)
                            ->update([
                                'exception_name' => $request->exception_name,
                        ...
                        if (count($countValidation) >= 1) {
                            ...
                            $termination_date = DB::table('TC_EXCEPTION_LISTS')
                            ->where(DB::raw('UPPER(ther_class_exception_list)'), strtoupper($request->ther_class_exception_list))
                            ->where(DB::raw('UPPER(ndc)'), strtoupper($request->ndc))
                            ->pluck('termination_date')->toArray();
                                ...
                                $addProviderValidationData = DB::table('TC_EXCEPTION_LISTS')
                                ->insert([
                                ...
                                $reecord = DB::table('TC_EXCEPTIONS')
                                    ->join('TC_EXCEPTION_LISTS', 'TC_EXCEPTIONS.ther_class_exception_list', '=', 'TC_EXCEPTION_LISTS.ther_class_exception_list')
                                    ->where('TC_EXCEPTION_LISTS.ther_class_exception_list', $request->ther_class_exception_list)
                                    // ->where('TC_EXCEPTION_LISTS.physician_id', $request->physician_id)
                                    ->first();
                                return $this->respondWithToken(
                                    $this->token(),

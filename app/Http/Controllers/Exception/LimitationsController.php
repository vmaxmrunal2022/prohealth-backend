            ...
            if($recordcheck){
                ...
                $insert = DB::table('LIMITATIONS_LIST')->insert(
                    [
                        ...
                        'MAX_DOSE'=>$request->max_rx_qty,
                        'DRUG_COV_START_DAYS'=>$request->min_rx_days,
                        'ACUTE_DOSING_DAYS'=>$request->max_rx_days,
                        'MIN_AGE'=>$request->min_ctl_days,
                        'MAX_AGE'=>$request->max_ctl_days,
                        'MAX_RXS_PATIENT'=>$request->max_refills,
                        'MAX_RXS_TIME_FLAG'=>$request->max_days_per_fill,
                        'MAX_PRICE_PATIENT'=>$request->max_dose,
                        'MAX_PRICE_TIME_FLAG'=>$request->min_age,
                        'MAX_REFILLS'=>$request->max_age,
                        'RETAIL_MAX_FILLS_OPT'=>$request->min_price,
                        'MAX_QTY_PER_FILL'=>$request->max_price,
                        'STARTER_DOSE_DAYS'=>$request->max_rxs_patient,
                        'STARTER_DOSE_BYPASS_DAYS'=>$request->max_price_patient,
                        'STARTER_DOSE_MAINT_BYPASS_DAYS'=>$request->generic_copay_amt,
                        'MAINT_DOSE_UNITS_DAY'=>$request->maint_dose_units_day,
                        'MAX_QTY_OVER_TIME'=>$request->acute_dosing_days,
                        'MAX_RX_QTY_OPT'=>$request->denial_override,
                        'RX_QTY_OPT_MULTIPLIER'=>$request->maintenance_drug,
                        'MAX_DAYS_OVER_TIME'=>$request->merge_defaults,
                        'MAX_DAYS_SUPPLY_OPT'=>$request->sex_restriction,
                        'MAIL_ORD_MAX_DAYS_SUPPLY_OPT'=>$request->mail_order_min_rx_days,
                        'MAIL_ORDER_MAX_REFILLS'=>$request->mail_order_max_rx_days,
                        'MAIL_ORD_MAX_FILLS_OPT'=>$request->mail_order_max_refills,
                        'VALID_RELATION_CODE'=>$request->max_rxs_time_flag,
                        'SEX_RESTRICTION'=>$request->max_price_time_flag,
                        'DAYS_PER_DISABILITY'=>$request->qty_dsup_compare_rule,
                        'DAYS_PER_DISABILITY_OPT'=>$createddate,
                        'MAX_PRICE_PER_DAY'=>'',
                        'MAX_PRICE_PER_DAY_OPT'=>$createddate,
                        'MAX_PRICE_PER_DIAG'=>$request->copay_network_ovrd,

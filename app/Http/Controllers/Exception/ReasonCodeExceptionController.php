...
class ReasonCodeExceptionController extends Controller
{
    public function add(Request $request)
    {
        ...
        if ($request->has('new')) {
            $validator = Validator::make($request->all(), [
                "reason_code_list_id" => ['required', 'max:10', Rule::unique('REASON_CODE_LISTS')->where(function ($q) {
            ...
            if ($validator->fails()) {
                return $this->respondWithToken($this->token(), 'error', $validator->errors(), "false");
            } else {
    ...
    public function getRejectCode(Request $request)
    {
        $reject_code = [
            ['reject_id' => '01', 'reject_label' => 'M/I Bin'],
            ['reject_id' => '02', 'reject_label' => 'M/I Version Number'],
            ['reject_id' => '03', 'reject_label' => 'M/I Transaction Code'],
            ['reject_id' => '04', 'reject_label' => 'M/I Procedure Control Number'],
            ['reject_id' => '05', 'reject_label' => 'M/I Pharmacy Number'],
            ['reject_id' => '06', 'reject_label' => 'M/I Group Number'],
            ['reject_id' => '07', 'reject_label' => 'M/I Cardholder ID Number'],
            ['reject_id' => '08', 'reject_label' => 'M/I Person Code'],
        ];
        return $this->respondWithToken($this->token(), '', $reject_code);
    }

    public function getReasonCode(Request $request)
    {
        $reason_code = DB::table('getReasonCode')
            ->select('REASON_CODE', 'REASON_DESCRIPTION')
            ->get();
        return $this->respondWithToken($this->token(), '', $reason_code);
    }
}

...
class NDCExceptionController extends Controller
{
    ...
    public function add1(Request $request)
    {
        ...
        if ($request->add_new == 1) {
            ...
            if ($validator->fails()) {
                ...
                $get_highest_termination_date=DB::table('NDC_EXCEPTION_LISTS')
                ->where('ndc_exception_list',$request->ndc_exception_list)
                
                ->pluck('termination_date')->toArray();

                $array_count=DB::table('NDC_EXCEPTION_LISTS')
                ->where('ndc_exception_list',$request->ndc_exception_list)
                ->where('ndc',$request->ndc)->get()->count();           // ->pluck('effe');
                // dd($array_count);
                ...
                if($effective_date_check == 1){
                    ...
                    else if (isset($get_highest_termination_date) && $request->effective_date>max($get_highest_termination_date)) {
                        $update = DB::table('NDC_EXCEPTION_LISTS')
                        ->insert([
                            'NDC_EXCEPTION_LIST' =>$request->ndc_exception_list,
                            'NDC'=>$request->ndc,
                    ...
                    return $this->respondWithToken($this->token(), 'Record Added Successfully', $update);
                     ...
                     return $this->respondWithToken($this->token(), 'Please Give Proper Effective Date');
    ...
    public function add(Request $request)
    {
        ...
        if ($request->new) {
            $validator = Validator::make($request->all(), [
                // 'physician_list' => ['required', 'max:10', Rule::unique('NDC_EXCEPTIONS')->where(function ($q) {
            ...
            if ($validator->fails()) {
                ...
                if (!$request->updateForm) {
                    ...
                    $ifExist = DB::table('NDC_EXCEPTIONS')
                        ->where(DB::raw('UPPER(ndc_exception_list)'), strtoupper($request->ndc_exception_list))
                        ->get();
                    if (count($ifExist) >= 1) {
                        return $this->respondWithToken($this->token(), [["NDC ID already exists"]], '', false);
                    }
                    // dd($ifExist);
                } else {
                }
                if ($request->ndc_exception_list && $request->ndc) {
                    $count = DB::table('NDC_EXCEPTIONS')
                        ->where(DB::raw('UPPER(ndc_exception_list)'), strtoupper($request->ndc_exception_list))
                    ...
                    if ($count <= 0) {
                        ...
                        $updateProviderExceptionData = DB::table('NDC_EXCEPTIONS')
                            ->where('ndc_exception_list', $request->ndc_exception_list)
                            ->update([
                                'exception_name' => $request->exception_name,
                        ...
                        $countValidation = DB::table('NDC_EXCEPTION_LISTS')
                            ->where(DB::raw('UPPER(ndc_exception_list)'), strtoupper($request->ndc_exception_list))
                            ->where(DB::raw('UPPER(ndc)'), strtoupper($request->ndc))
                            ->get();
                        ...
                        if (count($countValidation) >= 1) {
                            ...
                            $reecord = DB::table('NDC_EXCEPTIONS')
                                ->join('NDC_EXCEPTION_LISTS', 'NDC_EXCEPTIONS.ndc_exception_list', '=', 'NDC_EXCEPTION_LISTS.ndc_exception_list')
                                ->where('NDC_EXCEPTION_LISTS.ndc_exception_list', $request->physician_list)
                                // ->where('NDC_EXCEPTION_LISTS.physician_id', $request->physician_id)
                                ->first();
                            return $this->respondWithToken(
                                $this->token(),

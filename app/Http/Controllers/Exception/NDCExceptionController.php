...
class NDCExceptionController extends Controller
{
    ...
    public function ndcdelete(Request $request){
        ...
        elseif(isset($request->ndc_exception_list)){
            ...
            $get_exceptions_lists1=  DB::table('NDC_EXCEPTION_LISTS')
                             ->select('ndc')
                            ->where('ndc_exception_list', $request->ndc_exception_list)
                           
                            ->get();

            $get_exceptions_lists2=  DB::table('NDC_EXCEPTIONS')
                            ->where('ndc_exception_list', $request->ndc_exception_list)
                            ->first();
                            ...
                            // } else {
                                ...
                                foreach ($get_exceptions_lists1 as $excption) {
                                    ...
                                    $save_audit_delete1 = $this->auditMethod('DE', json_encode($excption), 'NDC_EXCEPTION_LISTS');
                            ...
                            // echo (json_encode($get_exceptions_lists2));
        ...
        // dd(json_encode($get_exceptions_lists2));
            ...
            if ($Exception) {
                return $this->respondWithToken($this->token(), 'ndc exceptopn Record Deleted Successfully',$Exception,true,201);
            } else {
                return $this->respondWithToken($this->token(), 'Record Not Found');
            }
        }
